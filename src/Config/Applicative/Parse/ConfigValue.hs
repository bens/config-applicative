...
findOne cfg rdr@(Reader psr _dom) info _ =
  fmap g <$> bindV (seek rdr key cfg) (traverse f)
  where
    key = optKey info
    f :: Config.Value Config.Position -> Validation [ParseError] (Parsed a)
    f = undefined
...
findMany
  ...
  -> Reader a -> Info a
  -> Maybe [a] -> Validation [ParseError] (Maybe [a])
findMany cfg rdr@(Reader psr _dom) info _ =
  undefined
...
findMap cfg rdr@(Reader psr _dom) info _ =
  undefined
  -- bindV (seek rdr key cfg) (traverse f)
  -- where
  --   key = optKey info
...
badParse (Reader _ dom) key v msg =
  undefined
  -- Failure [ConfigParseError (key, Config.valueAnn v) msg sample domain]
  -- where
  --   sample = Sample Nothing
...
recording1 (Reader _psr _dom) i (M (Compose (Compose m))) =
  undefined
  -- M (Compose (Compose (fmap f <$> m)))
  -- where
...
recordingN (Reader _psr _dom) i (M (Compose (Compose m))) =
  undefined
  -- M (Compose (Compose (fmap f <$> m)))
  -- where
  --   f (Pair (Const cfgs) (Failure e))  = Pair (Const cfgs) (Failure e)
...
recordingKV (Reader _psr _dom) i (M (Compose (Compose m))) =
  undefined
  -- M (Compose (Compose (fmap f <$> m)))
  -- where
  --   f (Pair (Const cfgs) (Failure e)) = Pair (Const cfgs) (Failure e)
